...
Example
-------
export ORDIQ_PREVIEW_URL=https://your-app-url.vercel.app
python -m cli \
       --purpose "Automated CV-screening" \
       --notes   "demo run" \
       --spec    docs/raw/annex_iii.csv \
       --pdf     reports/annex-iii-report.pdf
"""
...
def locate_wkhtmltopdf() -> str | None:
    ...
    try:
        ...
        pass
    for cand in ("/usr/bin/wkhtmltopdf", "/run/current-system/sw/bin/wkhtmltopdf"):
...
def pdf_via_reportlab(data: dict[str, Any], outfile: str) -> bool:
    ...
    w, h = A4; y = h - 50
    c.setFont("Helvetica-Bold", 16); c.drawString(40, y, "Compliance Assessment"); y -= 40
    c.setFont("Helvetica", 11)
    c.drawString(40, y, f"Matched key: {data.get('matched_key','N/A')}"); y -= 20
    c.drawString(40, y, f"Tier: {data.get('tier','N/A')}"); y -= 30
    c.setFont("Helvetica-Bold", 12); c.drawString(40, y, "Obligations:"); y -= 18
    c.setFont("Helvetica", 11)
    for o in data.get("obligations", []):
        c.drawString(60, y, f"• {o}"); y -= 15
        if y < 60:
            c.showPage(); y = h - 50
    c.save()
    print(f"✅  PDF saved → {outfile}")
...
def write_pdf(data: dict[str, Any], outfile: str) -> None:
    html = textwrap.dedent(f"""
      <!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/><title>Compliance Assessment</title>
      <style>body{{font-family:Arial,sans-serif;margin:2rem}}h1{{margin-top:0}}h3{{margin-bottom:.2rem;color:#444}}
      ul{{margin-top:.2rem}}</style></head><body>
        <h1>Compliance Assessment</h1>
        <p><b>Matched key:</b> {data.get("matched_key","N/A")}</p>
        <p><b>Tier:</b> {data.get("tier","N/A")}</p>
        <h3>Obligations</h3><ul>
...
def main(argv: list[str] | None = None) -> None:
    ...
    ap.add_argument("--purpose", required=True, help="Describe what the system does")
    ap.add_argument("--notes"  , default="", help="Free-text notes")
    ap.add_argument("--spec", "-s", required=True, help="Path to Annex III CSV spec")
